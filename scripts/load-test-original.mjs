...
async function runLoadTest(scenario) {
  ...
  while (Date.now() < endTime && totalStarted < config.targetUsers) {
    ...
    if (totalStarted % 1000 === 0) {
      const elapsed = Date.now() - stats.startTime;
      const rate = (stats.total / elapsed * 1000).toFixed(2);
  ...
  const p95Duration = stats.durations.length > 0 ? stats.durations.sort((a, b) => a - b)[Math.floor(stats.durations.length * 0.95)] : 0;
  const p99Duration = stats.durations.length > 0 ? stats.durations.sort((a, b) => a - b)[Math.floor(stats.durations.length * 0.99)] : 0;
