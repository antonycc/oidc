...
test.describe('OpenAPI Specification Compliance Tests', () => {
  ...
  test('/authorize endpoint validates required parameters', async ({ request }) => {
    ...
    const isErrorResponse = response.status() === 400 || 
      (response.status() === 200 && (await response.text()).includes('error'));
  ...
  test('API responses match OpenAPI examples', async ({ request }) => {
    ...
    const expectedExample = openApiSpec.paths['/.well-known/openid-configuration'].get.responses['200'].content['application/json'].example;
