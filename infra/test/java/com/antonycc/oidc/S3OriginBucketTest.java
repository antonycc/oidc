...
class S3OriginBucketTest {
  ...
  void s3OriginBucketCreatesExpectedResources() {
    ...
    Bucket logsBucket =
        Bucket.Builder.create(testStack, "LogsBucket")
    ...
    template.hasResourceProperties(
        ...
        Map.of(
            ...
            Map.of("Comment", "Test OAI comment for web bucket")));
  ...
  void s3OriginBucketWorksWithDifferentSuffixes() {
    ...
    S3OriginBucket wellKnownBucket =
        new S3OriginBucket(
            testStack,
    ...
    assertNotNull(
        wellKnownBucket.cachePolicy, "CachePolicy should be created for WELL_KNOWN bucket type");
    ...
    template.hasResourceProperties(
        ...
        Map.of(
            "CloudFrontOriginAccessIdentityConfig",
            Map.of("Comment", "Test OAI comment for well-known bucket")));
